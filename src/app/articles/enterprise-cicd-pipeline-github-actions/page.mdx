...
export const article = {
  ...
  date: '2026-02-17',
  title:
    'Enterprise CI/CD Pipeline: Multi-Project CDK Deployments with GitHub Actions and OIDC',
  description:
    'I deploy 4 CDK projects across 3 AWS accounts from a single GitHub repo using OIDC federation, a 465-line composite action, and environment-scoped Checkov scanning. 19 workflow files, zero long-lived credentials, and auto-rollback when smoke tests fail.',
}
...
> This article documents a production CI/CD system that deploys **4 CDK projects,
> 11 CloudFormation stacks, across 3 AWS accounts** from a single GitHub repository using
> **zero long-lived AWS credentials**. The pipeline splits into thin environment triggers,
> thick reusable workflows, and a shared composite action that handles bootstrap verification,
> SLSA provenance tagging, and stack output capture.
...
<ScreenshotPlaceholder
  alt="GitHub Actions CI pipeline overview showing all 9 jobs with change detection, lint, typecheck, build, test, and validation stages"
  type="screenshot"
/>
...
<Callout variant="insight" title="Why GitHub Actions Over CDK Pipelines">
  GitHub Actions' `workflow_call` gives the same composability without the
  CodePipeline overhead, and the `secrets: inherit` pattern allows
  environment-scoped secret resolution without declaring each secret explicitly.
  The result: **zero AWS infrastructure** for the CI/CD system itself.
</Callout>
...
<Callout variant="security" title="Zero Long-Lived Credentials">
  OIDC tokens are ephemeral (15-minute sessions), environment-scoped (different
  IAM roles per account), and fully auditable (CloudTrail shows the GitHub repo,
  workflow, and actor). There are **zero static access keys** stored anywhere in
  the system.
</Callout>
...
<Callout variant="warning" title="AWS-Imposed Constraint: us-east-1">
  The Next.js Edge stack (CloudFront, ACM, WAF) **must** deploy to `us-east-1`
  because CloudFront distributions require certificates and WAF WebACLs in that
  region. All other stacks deploy to `eu-west-1`. The workflow re-authenticates
  with the same OIDC role but a different region parameter.
</Callout>
...
sequence. Rather than duplicating this 465-line sequence across 11 different stack deployment
jobs, the composite action encapsulates it once:

<ProcessTimeline
  steps={[
    {
      title: 'Validate Inputs',
      description: 'Check environment, region, and 12-digit account ID format.',
    },
    {
      title: 'Verify AWS Credentials',
      description:
        'Run sts:GetCallerIdentity to confirm OIDC session is active.',
    },
    {
      title: 'Verify CDK Bootstrap (optional)',
      description:
        'Check CDKToolkit stack exists in the target account/region.',
    },
    {
      title: 'Build CDK Context',
      description: 'Construct -c project= and -c environment= flags.',
    },
    {
      title: 'Deploy with SLSA Provenance',
      description:
        'npx cdk deploy --method=direct --exclusively + 6 audit tags.',
    },
    {
      title: 'Capture Stack Outputs',
      description: 'Read CDK outputs-file and emit as job outputs (JSON).',
    },
  ]}
/>
...
<Callout variant="info" title="6 Audit Tags on Every Resource">
  Every deployed resource gets provenance tags automatically. These answer the
  question that comes up during every incident: "Which deployment changed this
  resource, and who triggered it?" With these tags, incident investigation
  becomes a tag lookup instead of a CloudTrail deep-dive.
</Callout>
...
<Callout variant="insight" title="Direct Deploy & Exclusive Stacks">
  `--method=direct` calls `UpdateStack`/`CreateStack` directly instead of
  creating changesets. This prevents `ChangeSetNotFoundException` race
  conditions when concurrent workflows (dev and staging) deploy simultaneously.
  `--exclusively` prevents CDK from auto-deploying dependency stacks — the
  workflow controls the explicit chain: **Data → Compute → Networking →
...
<Callout variant="security" title="Production Deployment Gate">
  The production trigger requires typing `"DEPLOY"` — not a checkbox toggle. A
  checkbox can be clicked absently, but typing `DEPLOY` requires deliberation.
  The `skip-staging-check` escape hatch exists for emergencies (hotfixes that
  need to bypass staging), but it defaults to `false`.
</Callout>
...
<ScreenshotPlaceholder
  alt="GitHub Actions deployment workflow showing sequential stack deployment with progress indicators for Data → Compute → Networking → Application → API → Edge stacks"
  type="screenshot"
/>
...
The Monitoring pipeline is the most complete deployment in the system. It runs the full
lifecycle end to end: security scan, drift check, sequential deploy, verification, smoke
tests, and auto-rollback if something breaks.
...
### The Monitoring Deployment Lifecycle

<ProcessTimeline
  steps={[
    {
      title: 'Setup & Synthesize',
      description:
        'Run synthesize-ci.ts to produce CDK cloud assembly and emit stack names to GITHUB_OUTPUT.',
    },
    {
      title: 'Security Scan + Drift Detection',
      description:
        'Checkov scans synthesized templates (conditional). cdk diff checks for manual drift (staging/prod only).',
    },
    {
      title: 'Deploy Storage & SSM (parallel)',
      description:
        'EBS + DLM lifecycle policies deploy in parallel with SSM Run Command documents and scripts.',
    },
    {
      title: 'Deploy Compute',
      description:
        'EC2/ASG with security groups — depends on both Storage and SSM completing successfully.',
    },
    {
      title: 'Verify Deployment',
      description:
        'verify-deployment.ts uses AWS SDK to confirm stack outputs, instance health, and resource states.',
    },
    {
      title: 'Smoke Tests via SSM',
      description:
        'SSM Run Command executes health checks directly on the instance — no SSH, no public endpoints.',
    },
    {
      title: 'Rollback or Summary',
      description:
        'On failure: auto-rollback Compute to last known-good template (staging/prod only). On success: generate deployment summary.',
    },
  ]}
/>
...
<Callout variant="warning" title="Auto-Rollback: Only When It Makes Sense">
  The rollback only fires when the deploy **succeeded** but verification
  **failed** — meaning the new infrastructure is broken. Development is excluded
  because failed dev deployments are learning opportunities, not incidents. If
  the deploy itself failed, CloudFormation already rolled back automatically.
</Callout>
...
<Callout variant="info" title="Progressive Security Severity">
  Security scan severity escalates with environment criticality. Development
  findings never block — they're informational. Staging blocks on HIGH and
  CRITICAL. Production blocks on **everything**, including LOW severity. Dev is
  for experimenting, staging is for catching mistakes, and production gets zero
  tolerance.
</Callout>
...
<ScreenshotPlaceholder
  alt="Checkov security scan output showing CloudFormation template analysis with passed, failed, and skipped checks across multiple resource types"
  type="terminal"
/>
...
<ScreenshotPlaceholder
  alt="SSM Run Command smoke test output showing container health checks for Grafana, Prometheus, Node Exporter, and Loki with pass/fail status indicators"
  type="terminal"
/>
...
<Callout variant="insight" title="$0/Month CI/CD — By Design, Not By Accident">
  The entire pipeline system — 19 workflows, 2 composite actions, security
  scanning, smoke tests, and auto-rollback — operates within GitHub's free tier
  (2,000 min/month). The only AWS cost is the resources being deployed, not the
...
The three-tier structure (thin triggers → thick reusables → composite action) has held up
well. Adding the Org project took one trigger file and 3 lines in `stacks.ts`. The SLSA
tags paid for themselves twice during incident response — I found the offending commit
in seconds instead of digging through CloudTrail. The auto-rollback caught a real staging
failure before I even opened the GitHub Actions UI.

The gaps are mostly around visibility: SARIF integration for Checkov findings in the Security
tab, DORA metrics for tracking deployment frequency and change failure rate, and a staging-to-prod
promotion gate that would let smoke test results automatically unlock production. The pipeline's
composition model means each of these can be added as a new reusable workflow without touching
the existing deployment logic.
