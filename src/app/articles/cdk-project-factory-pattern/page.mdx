...
export const article = {
  ...
  date: '2026-02-17',
  title:
    'Enterprise CDK Architecture: The Project Factory Pattern — Managing 4 Projects and 11 Stacks from a Single Entry Point',
  description:
    'How a solo DevOps engineer built a construct-to-factory pipeline with typed config modules, SSM-based cross-stack discovery, generic factory interfaces, CDK Aspects for governance, and a shared L3 construct library — all orchestrated by a slim entry point that delegates everything to project-specific factories.',
}
...
> shows how a slim entry point delegates 4 projects, 11 stacks, and ~300 resources to
> project-specific factories — where typed config modules, SSM-based discovery, and CDK
> Aspects replace the usual tangle of `app.ts` orchestration.
...
L3 constructs are your organisation's opinionated blueprints — they compose multiple L2 constructs into a single unit with validated defaults, helper methods, and CDK-Nag compliance baked in. The `lib/common/` directory contains L3 constructs organised by domain:
...
<Callout
  variant="insight"
  title="Why L3 Constructs Are Your Internal Developer Platform"
>
  ...
  environment variable merging — _without the stack author knowing_. That's the
  golden path concept from Platform Engineering: the right thing is the easy
  thing.
</Callout>
...
The factory pattern costs nothing at runtime — it's synthesis-time TypeScript, not deployed infrastructure. What it saved was roughly 2,000 lines of duplicate constructs, a significant reduction in `app.ts` complexity, and 3 security regressions caught by CDK-Nag and the DynamoDB aspect before they reached production. Maintenance runs about 1–2 hours per month, mostly construct library updates in `lib/common/` that automatically benefit every project. Adding a new stack is additive — you add it to the factory's `createAllStacks()` method — rather than invasive.
...
> infrastructure that usually requires a team.**
