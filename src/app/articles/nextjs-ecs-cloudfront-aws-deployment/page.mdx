...
export const article = {
  ...
  description:
    'How a solo DevOps engineer containerized a Next.js portfolio application across ECS on EC2, CloudFront with WAF, API Gateway with Lambda, DynamoDB, and S3 — using a consolidated CDK stack architecture with observability sidecars and auto-deploy from ECR pushes.',
}
...
<Callout variant="security" title="Defense-in-Depth: Dual WAF Protection">
  ...
  CloudFront WAF. This ensures both edge and direct-access traffic are protected
  by rate limiting and AWS managed rule groups — a true defense-in-depth
  pattern.
</Callout>
