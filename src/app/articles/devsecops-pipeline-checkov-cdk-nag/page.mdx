...
export const article = {
  ...
  description:
    'How a solo DevOps engineer implemented policy-as-code with 30 custom Python security checks across 11 domain-grouped files, environment-specific severity gating, 4 compliance frameworks, and GitHub Security tab integration — catching IMDSv1 bugs before they reach CloudFormation.',
}
...
The entire security pipeline runs on open-source tooling at zero incremental cost — Checkov, CDK-Nag, GitHub Actions (within free-tier limits), and SARIF uploads are all free for public repositories. The only ongoing investment is authoring time: adding a new Checkov check means adding a class to the appropriate domain-grouped file in `.checkov/custom_checks/` (auto-discovered, no config changes), adding a CDK-Nag suppression requires inline metadata or an entry in `COMMON_SUPPRESSIONS` with a mandatory `reason` string, and enabling additional compliance packs is a one-line change to the `applyCdkNag()` call.
...
> tags at deploy — creates a defence-in-depth posture where security findings must be
