...
export const article = {
  ...
  description:
    'How a solo DevOps engineer built a production-grade monitoring platform — with 7 Docker containers, 9 Grafana dashboards, EC2 + Cloud Map service discovery, and zero public ingress — all deployed via SSM Run Command from CDK.',
}
...
The monitoring platform had to meet six non-negotiable requirements: scrape metrics from ECS tasks with dynamic IPs, aggregate logs from multiple ECS services and the EC2 host, receive distributed traces from the Next.js application, survive EC2 instance replacements without losing historical data, expose zero public network endpoints (SSM-only access), and deploy entirely from CDK with no manual SSH configuration.
...
The direction is clear: the monitoring _data plane_ (Prometheus, Loki, Tempo) may eventually move to managed backends (AMP, Grafana Cloud) as the portfolio grows. The monitoring _control plane_ (dashboards, alert rules, datasource config) will stay in Git and CDK. The value of this project is not the containers themselves — it's the patterns for service discovery, bootstrap, persistence, and zero-trust access that transfer to any monitoring architecture.
...
_Building cloud infrastructure in Dublin, applying FinOps principles to every line of CDK. If you're questioning whether your architecture is over-engineered for your scale, reach out._
