...
import { submitPipelineAction } from '@/lib/api/admin-api'
import type { PipelineActionResponse } from '@/lib/api/admin-api'
...
export function usePipelineAction() {
  ...
  return useMutation<PipelineActionResponse, Error, PipelineActionParams>({
    mutationFn: ({ slug, action }: PipelineActionParams) =>
      submitPipelineAction(slug, action),
