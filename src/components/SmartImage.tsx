...
async function fetchSidecar(
  ...
  try {
    const res = await fetch(sidecarUrl, {
      next: { revalidate: 3600 }, // ISR: refetch sidecar every hour
    })
          ...
          {...(sidecar?.blurHash
            ? {
                ...
                blurDataURL: `data:image/svg+xml;base64,${Buffer.from(
                  `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><rect width="100%" height="100%" fill="#71717a"/></svg>`,
                ).toString('base64')}`,
              }
            : {})}
        />
